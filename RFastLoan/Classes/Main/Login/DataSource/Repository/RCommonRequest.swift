...
enum RUploadContactsApi {
    case uploadContacts()
}

extension RUploadContactsApi : URLConvertible {
    func asURL() throws -> URL {
        return URL.init(string: hostUrlStr + "loanManage/api/userTel/v1/uploadUserTel")!
    }
}
...
//                multipartFormData.append(imageData, withName: "fileupload[\(index)]", fileName: fileName, mimeType: "image/jpeg")
    ...
    func uploadContacts(contacts: Data, success: @escaping (_ response : Dictionary<String, String>) -> (), failture : @escaping (_ error : Error)->()) {
